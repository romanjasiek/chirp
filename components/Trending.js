  ...
  return (
    <div className="hover:bg-white hover:bg-opacity-[0.03] px-4 py-2 cursor-pointer transition duration-200 ease-out flex items-center justify-between">
      <div className="space-y-0.5">
        <p className="text-[#888888] text-xs font-medium">{result.heading}</p>
        <h6 className="font-bold max-w-[250px] text-sm">
          {result.description}
        </h6>
        <p className="text-[#888888] text-xs font-medium max-w-[250px]">
          Trending with{" "}
      ...
      {result.img ? (
        ...
        <div className="icon group">
          <DotsHorizontalIcon className="h-5 text-[#888888] group-hover:text-[#E9C46A]" />
        </div>
      )}
    </div>
  );
